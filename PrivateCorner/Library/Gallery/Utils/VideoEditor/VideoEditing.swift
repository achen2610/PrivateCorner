...
public protocol VideoEditing: class {
  ...
  func edit(video: GVideo, completion: @escaping (_ video: GVideo?, _ tempPath: URL?) -> Void)
}

extension VideoEditing {

  public func process(video: GVideo, completion: @escaping (_ video: GVideo?, _ tempPath: URL?) -> Void) {
  ...
  func handle(outputURL: URL, completion: @escaping (_ video: GVideo?, _ tempPath: URL?) -> Void) {
    ...
    PHPhotoLibrary.shared().performChanges({
      ...
      localIdentifier = request?.placeholderForCreatedAsset?.localIdentifier
        ...
        if let localIdentifier = localIdentifier, let asset = Fetcher.fetchAsset(localIdentifier) {
          completion(GVideo(asset: asset), outputURL)
        } else {
          completion(nil, outputURL)
        }
    })
